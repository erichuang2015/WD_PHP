...
{				
	...
	function Check_Out() {
		...
		return $arFeedback = ECPay_Send::CheckOut($arParameters, $this->HashKey, $this->HashIV, ECPay_PaymentMethod::Credit, $this->ServiceURL);
	}
	...
	*/
	function CheckOut_App($arPostData){
		...
		$this->Send['TradeType']            = 1;
	        ...
	        $arParameters = array_merge( array('MerchantID' => $this->MerchantID) ,$this->Send);
		$arFeedback = ECPay_Send::CheckOut($arParameters, $this->HashKey, $this->HashIV, ECPay_PaymentMethod::Credit, $this->ServiceURL);
	...
	//查詢信用卡單筆明細紀錄
	function QueryTrade(){
		return $arFeedback = ECPay_QueryTrade::CheckOut(array_merge($this->Trade, array('MerchantID' => $this->MerchantID)), $this->HashKey, $this->HashIV, $this->ServiceURL);
	}
...
*  送出資訊
*/
class ECPay_Send extends ECPay_IO
{
	...
	protected static function process_send($arParameters = array(), $HashKey = '', $HashIV = '', $Payment_Method = '', $ServiceURL = ''){
		...
		//宣告物件
		$PaymentMethod    = 'ECPay_'.$Payment_Method;
...
// 訂單資料查詢
class ECPay_QueryTradeInfo extends ECPay_IO
{
    static function CheckOut($arParameters = array(), $HashKey ='', $HashIV ='', $ServiceURL = ''){
        ...
        {
            $arParameters["CheckMacValue"] = ECPay_CheckMacValue::generate($arParameters, $HashKey, $HashIV);
            ...
            {
                $szConfirmMacValue = ECPay_CheckMacValue::generate($arConfirmArgs, $HashKey, $HashIV);
...
// 信用卡關帳/退刷/取消/放棄
class ECPay_DoAction extends ECPay_IO
{
    static function CheckOut($arParameters = array(), $HashKey ='', $HashIV ='', $ServiceURL = ''){
	...
	// 變數宣告。
        ...
        //產生驗證碼
        $szCheckMacValue = ECPay_CheckMacValue::generate($arParameters,$HashKey,$HashIV);
...
// 查詢信用卡單筆明細記錄
class ECPay_QueryTrade extends ECPay_IO
{
    static function CheckOut($arParameters = array(), $HashKey ='', $HashIV ='', $ServiceURL = ''){
        $arErrors = array();
        $arFeedback = array();
...
// 下載信用卡撥款對帳資料檔
class ECPay_FundingReconDetail extends ECPay_IO
{   
...
// 驗證廠商憑證
class ECPay_Verify_Vendor extends ECPay_IO
{
...
// 產生apple pay buttom
class ECPay_Apple_Button extends ECPay_IO{
...
Abstract class ECPay_Verification
{
	...
	*/
	function generate_checkmacvalue($arParameters = array(), $HashKey = '', $HashIV = ''){
		...
		$sCheck_MacValue = ECPay_CheckMacValue::generate($arParameters, $HashKey, $HashIV);

		return $sCheck_MacValue ;
	}

    	function generate_encrypt_data($sPaymentToken, $HashKey, $HashIV){
    		return ECPay_CheckMacValue::encrypt_data($sPaymentToken, $HashKey, $HashIV);
    	}
...
class ECPay_CheckMacValue
{
	...
	*/
	static function encrypt_data($sPost_Data = '', $sKey = '', $sIv = ''){
		...
		$sPost_Data = ECPay_CheckMacValue::addpadding($sPost_Data);   					// PaddingMODE
	...
	*/
	static function decrypt_data($sPost_Data = '', $sKey = '', $sIv = ''){
		...
		$decrypted = mcrypt_decrypt( MCRYPT_RIJNDAEL_128, $sKey, $sPost_Data, MCRYPT_MODE_CBC, $sIv);	//ace解碼
		$decrypted = ECPay_CheckMacValue::stripPkcs7Padding($decrypted);				// 除去pkcs7
		
		return $decrypted ;
	}

	//Padding PKCS7的Function
	static function addpadding($string){
		//Pkcs7
